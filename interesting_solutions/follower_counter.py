...
#
# Гарантируется, что количество уникальных комментаторов у всех наших героев разное. Могут быть ситуации,
# когда у героя нету ни единого комментатора, в таком случае все равно нужно выводить информацию о нем.
#
# Sample Input 1:
# Дили: navalny
...
d = {blogger: set() for blogger in ('Били', 'Дили', 'Вили')}
for blogger, follower in map(lambda x: x.split(': '), iter(input, 'конец')):
    d[blogger].add(follower)

for blogger, followers in sorted(d.items(), key=lambda x: -len(x[1])):
    print(f'Количество уникальных комментаторов у {blogger} - {len(followers)}')