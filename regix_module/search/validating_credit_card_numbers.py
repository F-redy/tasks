...
pattern = re.compile(r"""(?x)
    (?!.*(\d)(-?\1){3,})           # (negative lookahead) нет последовательности цифр повторяющихся три или более раза
    (?=(?:^[456]))                 # (positive lookahead) строка начинается с одной из цифр [456]
    \d{4}-?\d{4}-?\d{4}-?\d{4}$    # основное выражение ищет по шаблону 4123456789123456 или 5133-3367-8912-3456
""")
